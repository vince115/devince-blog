...
// ✅ rehype plugin：自動把 class → className
    ...
    return (tree: Root) => {
        visit(tree, "element", (node: Element) => {
            ...
            if (props && "class" in props) {
                ...
                delete (props as Record<string, unknown>).class;
            }
        });
    };
}
...
export const Project = defineDocumentType(() => ({
    name: "Project",
    filePathPattern: `projects/**/*.mdx`, // ✅ 改成 .mdx，統一規格
    contentType: "mdx",
    fields: {
        title: { type: "string", required: true },
        date: { type: "date", required: true },
        ...
        image: { type: "string" },
        url: { type: "string" },
        externalUrl: { type: "string" }, // 可用來記錄外部連結
        cover: { type: "string" }, // ✅ 新增這行
    },
    computedFields: {
        slug: { type: "string", resolve: (d) => d._raw.sourceFileName.replace(/\.mdx?$/, "") },
...
export const Author = defineDocumentType(() => ({
    name: "Author",
    filePathPattern: `authors/*.mdx`,        // ✅ 這行改對了
    contentType: "mdx",                     // ✅ 要支援 <br /> 等標籤
    fields: {
